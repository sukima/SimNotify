...
# {shared_path}/database.yml on the remote server, thus eliminating
...
Capistrano::Configuration.instance.load do
  ...
  DESC
      task :db, :roles => :db, :only => { :primary => true } do
        ...
        # Download dump
        download "#{shared_path}/sync/#{filename}", File.join("sync", filename)
  ...
  DESC
      task :fs, :roles => :web, :once => true do
        ...
        Array(fetch(:sync_directories, [])).each do |syncdir|
          ...
          system "tar cjf sync/#{base}.#{Time.now.strftime '%Y-%m-%d_%H:%M:%S'}.tar.bz2 #{syncdir}"
        end
      ...
      # task :fs, :roles => :web, :once => true do
        ...
        # Array(fetch(:sync_directories, [])).each do |syncdir|
          ...
          # else
            ...
            # run "mkdir -p #{current_path}/#{syncdir}"
          # end
